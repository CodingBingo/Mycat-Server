...
public class XMLDatabaseRuleLoader {
    private final static String DEFAULT_DTD = "/dbRule.dtd";
    private final static String DEFAULT_XML = "/dbRule.xml";
    ...
    public XMLDatabaseRuleLoader(String ruleFile) {
        // this.rules = new HashSet<RuleConfig>();
        //rule名 -> rule
        this.databaseRules = new HashMap<String, DatabaseRuleConfig>();
        //function名 -> 具体分片算法
        this.functions = new HashMap<String, AbstractPartitionAlgorithm>();
        load(DEFAULT_DTD, ruleFile == null ? DEFAULT_XML : ruleFile);
    }

    public XMLDatabaseRuleLoader() {
        this(null);
    }

    public Map<String, DatabaseRuleConfig> getDatabaseRules() {
        return (Map<String, DatabaseRuleConfig>) (databaseRules.isEmpty() ? Collections
                .emptyMap() : databaseRules);
    }
     ...
     */
    private void loadDatabaseRules(Element root) throws SQLSyntaxErrorException {
        //获取每个tableRule标签
        NodeList list = root.getElementsByTagName("databaseRule");
        for (int i = 0, n = list.getLength(); i < n; ++i) {
            ...
            if (node instanceof Element) {
                ...
                if (databaseRules.containsKey(name)) {
                ...
                //保存到tableRules
                databaseRules.put(name, new DatabaseRuleConfig(name, rule));
            }
        }
    }
